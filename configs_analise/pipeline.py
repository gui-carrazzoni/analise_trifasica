...
def _carregar_sinais(
    ...
    if cfg.fonte == "comtrade":
        logger.info("▶️  1/4 Lendo registro COMTRADE...")
        df_sin, df_raw, meta, cfg = carregar_sinais_comtrade(cfg)
        return df_sin, cfg, meta, df_raw
    logger.info("▶️  1/4 Gerando sinais trifásicos simulados (Cold Load Pickup)...")
...
def executar_simulacao_protecao(
    ...
    logger.info("▶️  2/4 Estimando fasores H1 e H2 (DFT deslizante)...")
    df_fas = estimar_fasores(df_sin, cfg)

    logger.info("▶️  3/4 Calculando correntes diferenciais (H1)...")
...
def exportar_resultados(resultados: ResultadoPipeline, cfg: Config) -> None:
    ...
    logger.info("✅ Resultados exportados para CSV com sucesso.")
